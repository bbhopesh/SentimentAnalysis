...
def get_raw_output_file_path():
    return os.path.join(get_results_dir_path(), "tuning_raw_output_svm2.txt")


def get_tuning_results_file_path():
    return os.path.join(get_results_dir_path(), "tuning_output_svm2.json")
...
def main():
    ...
    with open(get_raw_output_file_path(), "w+") as raw_output:
        ...
        for k in relevant_feature_sizes:
            ...
            best_params = svm(train_X, train_y, k, raw_output)


            print_output("\tStarting training.", raw_output)
            perc = fac.svm_factory(best_params)
            perc.fit(test_X,test_y)
