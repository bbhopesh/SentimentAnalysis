...
def get_raw_output_file_path():
    return os.path.join(get_results_dir_path(), "tuning_raw_output.txt")


def get_tuning_results_file_path():
    return os.path.join(get_results_dir_path(), "tuning_output.json")
...
def main():
    ...
    with open(get_raw_output_file_path(), "w+") as raw_output:
        ...
        for k in relevant_feature_sizes:
            ...
            import sys
            print_output(train_X.shape,sys.stdout)
            print_output("\tRead and trasformed data.", raw_output)
            perceptron(train_X, train_y, k, raw_output)
            logistic_regression(train_X, train_y, k, raw_output)
            multinomial_naive_bayes(train_X, train_y, k, raw_output)
            decision_tree(train_X, train_y, k, raw_output)
            svm(train_X, train_y, k, raw_output)
            '''
